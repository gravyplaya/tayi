...
class ChatApiController extends Controller
{
   ...
   public function all_framework_chats(Request $request)
    { 
        $f_id= $request->framework_id;
        $framework_details=Framework::with('chat_questions')->get();
         return response()->json(['frameworks'=>$framework_details]);
    }
    ...
    { 
        $check_chat=Chat::where('chat_id',$request->chat_session)->first();
        if($check_chat){
            $chats=Chat::where('chat_id',$request->chat_session)->get();
        }else{
        $f_id= $request->framework_id;
